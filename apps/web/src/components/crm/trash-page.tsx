...
export const TrashPage = ({ action, records, resource, teamSlug }: Readonly<{ action: CrmMutationAction; records: readonly TrashRecord[]; resource: CrmResource; teamSlug: string }>) => {
    ...
    return <>
        ...
        <section className={styles.panel}><h2>Deleted {resource}</h2>{records.length === 0 ? <div className={styles.empty}><strong>Trash is empty</strong></div> : <ul className={styles.recordList}>{records.map((record) => <li key={record.id}><div className={styles.recordMark}>{record.title.slice(0, 1)}</div><div className={styles.recordBody}><strong>{record.title}</strong><span>Deleted {new Intl.DateTimeFormat("en", { dateStyle: "medium" }).format(new Date(record.deletedAt))}</span></div><form action={formAction}><input type="hidden" name="intent" value="restore" /><input type="hidden" name="id" value={record.id} /><button type="submit">Restore</button></form></li>)}</ul>}</section>
    </>;
};
