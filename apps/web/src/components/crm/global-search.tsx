...
export const GlobalSearch = ({ teamSlug }: Readonly<{ teamSlug: string }>) => {
    ...
    return <><button type="button" className={styles.searchButton} onClick={() => setOpen(true)}>Search <kbd>⌘K</kbd></button>{open ? <div className={styles.searchBackdrop} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}><section className={styles.searchDialog} role="dialog" aria-modal="true" aria-label="Search workspace"><input autoFocus aria-label="Search companies, people, opportunities, tasks, and notes" value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "ArrowDown") { event.preventDefault(); setActive((value) => Math.min(results.length - 1, value + 1)); } if (event.key === "ArrowUp") { event.preventDefault(); setActive((value) => Math.max(0, value - 1)); } if (event.key === "Enter") document.getElementById(`search-result-${active}`)?.click(); }} placeholder="Search every record…" /><div className={styles.searchResults}>{query.length < 2 ? <p>Type at least two characters.</p> : results.length === 0 ? <p>No matching records.</p> : results.map((result, index) => <Link id={`search-result-${index}`} data-active={active === index} key={`${result.resource}-${result.id}`} href={`/app/${teamSlug}/${result.resource}/${result.id}`} onClick={close}><span><strong>{result.title}</strong><small>{result.context}</small></span><em>{result.resource}</em></Link>)}</div></section></div> : null}</>;
};
